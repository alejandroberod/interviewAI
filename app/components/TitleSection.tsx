...
export default function TitleSection({title, subtitle, button=false}:{title:string, subtitle:string, button?:boolean}) {
  return (
    <div className="flex flex-col gap-2 md:gap-3 md:items-start justify-center">
      ...
      <h2 className="text-xl md:text-4xl lg:text-4xl font-bold text-white">{title}</h2>
      <p className="font-extralight text-gray-300 md:text-lg">{subtitle}</p>
      {button && <Button className="md:mt-7 lg:text-lg lg:py-8">Create Interview</Button>}
    </div>
  )
}
